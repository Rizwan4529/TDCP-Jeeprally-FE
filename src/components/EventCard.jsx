...
const Card = ({ image, name, location, date, desc }) => {
  return (
    <div className='bg-[#EFF4EF] rounded-lg shadow-md p-4 relative spacing-y-sm'>
      <img
        src={image}
        alt={name}
      ...
      <p className='absolute top-6 left-0 bg-accent text-white text-sm px-3 py-1'>{date}</p>
      ...
      <div className='spacing-y-sm px-1'>
        ...
        <p className='text-sm text-muted flex items-center gap-px'>
          <LOCATION_ICON className='icon text-accent shrink-0' />
        ...
        <div
          className='relative inline-flex items-center gap-2 text-accent hover:text-primary transition-all duration-300 text-sm cursor-pointer group'
        >
